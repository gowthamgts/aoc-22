...
func NewMonkey(matches []string) *Monkey {
	...
	m.divisor, _ = strconv.Atoi(matches[2])
	m.success, _ = strconv.Atoi(matches[3])
	m.fail, _ = strconv.Atoi(matches[4])
		...
		for _, m := range monkeys {
			for _, item := range m.items {
				var (
					...
					roundedValue = int(math.Round(float64(newLevel / 3)))
				)

				if roundedValue%m.divisor == 0 {
					monkeys[m.success].items = append(monkeys[m.success].items, roundedValue)
				} else {
					monkeys[m.fail].items = append(monkeys[m.fail].items, roundedValue)
				}
